...
// Puzzle pool per length — noun/adjective/adverb/infinitive only (Lettres Croisées, Mot Secret).
// Deep enough that a daily walk through the pool lasts years without reusing a word.
const PUZZLE_QUOTA = { 3: 400, 4: 900, 5: 1200, 6: 1400, 7: 1300, 8: 900 };
...
// Front-coding: since the list is sorted, each word ships as the length of the prefix it
// shares with the one before (a single 0-9a-z char) plus its own suffix. Halves the raw
// bytes, and words/index.ts decodes it back in parseWords().
...
function frontCode(words) {
	...
	for (const w of words) {
		let n = 0;
		while (n < prev.length && n < w.length - 1 && prev[n] === w[n]) n++;
		out.push(PREFIX_DIGITS[n] + w.slice(n));
		prev = w;
	}
	return out;
}
...
function emit(name, words) {
	const lines = [];
	let cur = '';
	for (const t of frontCode(words)) {
		if (cur.length + t.length + 1 > 100) { lines.push(cur); cur = t; }
