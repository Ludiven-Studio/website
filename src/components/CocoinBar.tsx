...
/* Compact wallet reminder: balance, the daily reward when it's claimable, and a way
   ...
   Prefix is `cw-`: these styles are global, and `cb-` collided with Casse-Briques. */
...
.cw-bal.pop { animation: cw-pop 0.7s cubic-bezier(0.2, 1.6, 0.4, 1); }
@keyframes cw-pop {
  0% { transform: scale(1); }
...
.cw-claim:hover { filter: brightness(1.06); }
.cw-shop {
...
.cw-shop:hover { border-color: var(--accent-regular); color: var(--accent-regular); }
@media (prefers-reduced-motion: reduce) { .cw-bal.pop { animation: none; } }
`;
