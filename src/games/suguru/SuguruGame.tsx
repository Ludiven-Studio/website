...
export default function SuguruGame({ gameId }: { gameId: string }) {
	...
	const wrongCells = useMemo(() => {
		...
		for (let r = 0; r < size; r++)
			for (let c = 0; c < size; c++) {
				const v = entries[r][c];
				if (given[r][c] == null && v != null && v !== solution[r][c]) out.push([r, c]);
			}
		return out;
	}, [entries, given, solution, size]);
	...
	const placeValue = useCallback(
		...
		[status, revealed, selected, given, zones, zoneSize, started, daily, gameId, lv.playing],
	);
		...
		<div className="sg-root" style={{ ['--n' as string]: size }}>
			...
			{status !== 'won' && !revealed && !(lv.active && !lv.playing) && (
				<div className="sg-actions">
					<button className="sg-act" onClick={hint} disabled={!gate.ready || (timed && !started)}>{gate.label}</button>
			...
			<div className="sg-boardwrap" style={{ ['--n' as string]: size }}>
				...
				<div
					className={`sg-board ${(daily || lv.playing) && !started ? 'blurred' : ''}`}
				...
				{((daily && !dailyLoading) || lv.playing) && !started && status !== 'won' && (
					<div className="sg-overlay">
						<button className="sg-startbtn" onClick={startTimer}>
...
.sg-root {
  ...
  --sg-given: color-mix(in srgb, var(--gray-0) 12%, transparent);
...
/* Fixed clues carry a tint on top of whatever the cell's background is, so the selection
   and conflict highlights still read through it. Colour alone was too easy to miss. */
.sg-cell.given {
  color: var(--gray-0);
  cursor: default;
  background-image: linear-gradient(var(--sg-given), var(--sg-given));
}
