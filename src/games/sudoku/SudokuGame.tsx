...
export default function SudokuGame({ gameId }: { gameId: string }) {
	...
	const giveUp = useCallback(() => {
		const solved = puzzle.solution.map((row) => [...row]);
		const sd = dailySeedRef.current;
		setEntries(solved);
		setSelected(null);
		setRevealed(true);
...
.game-page.gf-full .sk-root { max-width: none; width: 100%; height: 100%; }
.game-page.gf-full .sk-boardwrap { flex: 1; min-height: 0; max-width: none; container-type: size; display: flex; align-items: center; justify-content: center; }
.game-page.gf-full .sk-board { width: min(100cqw, 100cqh); }
.sk-boardwrap {
  position: relative;
  width: 100%;
