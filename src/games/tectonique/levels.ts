...
export const tectoniqueLevels: LevelPlan<TectoniqueLevelCfg> = {
	...
	config(level: number): TectoniqueLevelCfg {
		...
		return {
			...
			colLocks: locks,
			allLocks: l >= 70 ? 1 : 0,
			rocks: Math.min(6, 1 + Math.floor((l - 1) / 15)),
			holes: n - 3,
			seed: levelSeed(l),
			threeStar: three,
			twoStar: three * 2,
...
export const TECTONIQUE_BANDS: GenParams[] = [
	{ n: 6, crystals: 4, rowLocks: 0, colLocks: 0, allLocks: 0, rocks: 2, holes: 3 },
	{ n: 8, crystals: 6, rowLocks: 1, colLocks: 1, allLocks: 0, rocks: 4, holes: 5 },
	{ n: 10, crystals: 8, rowLocks: 2, colLocks: 2, allLocks: 1, rocks: 6, holes: 7 },
];
